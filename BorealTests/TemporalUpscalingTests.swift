...
struct TemporalUpscalingTests {
    ...
    @Test func optiScalerReleaseLayoutIsImportedWithoutInternalManifestAndRestored() async throws {
        ...
        try makePE(architecture: .x86_64).write(to: game.appending(path: "game.exe"))
        ...
        #expect(receipt.replacedFiles.map(\.relativePath) == ["OptiScaler.ini", "OptiScaler/shaders/shader.dll", "version.dll"])
        ...
        #expect(FileManager.default.fileExists(atPath: game.appending(path: "version.dll").path))
