...
script_source="${0:A:h}/BorealPhoneDE.js"
config_source="${0:A:h}/BorealPhoneDE.ini"
...
mkdir -p "$cleo_root"
destination="$cleo_root/BorealPhoneDE[fs].js"

if [[ -e "$destination" ]] && ! cmp -s "$script_source" "$destination"; then
  backup="$destination.backup-$(date +%Y%m%d-%H%M%S)"
  cp -p "$destination" "$backup"
  print "Previous Boreal Phone DE backed up to $backup"
fi

cp "$script_source" "$destination"
config_destination="$cleo_root/BorealPhoneDE.ini"
...
print "Boreal Phone DE installed at $destination"
print "Press F8 in-game to open the phone. Press F11 to reload BorealPhoneDE.ini."
