...
script_source="${0:A:h}/PolicePursuitRadar.js"
config_source="${0:A:h}/PolicePursuitRadar.ini"
...
mkdir -p "$cleo_root"
destination="$cleo_root/PolicePursuitRadar[fs].js"

if [[ -e "$destination" ]] && ! cmp -s "$script_source" "$destination"; then
  backup="$destination.backup-$(date +%Y%m%d-%H%M%S)"
  cp -p "$destination" "$backup"
  print "Previous Police Pursuit Radar backed up to $backup"
fi

cp "$script_source" "$destination"
config_destination="$cleo_root/PolicePursuitRadar.ini"
...
print "Police Pursuit Radar installed at $destination"
print "Press F11 in-game to reload PolicePursuitRadar.ini."
